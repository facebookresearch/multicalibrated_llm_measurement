"""
LLM inference pipeline for CAP political text classification.

Classifies political texts as law/crime-related (CAP topic 12)
using Llama 3.1 via HuggingFace Transformers + PyTorch.
...
Works on:
    - NVIDIA GPUs (A100, H100, etc.) via CUDA
    - Apple Silicon (CPU fallback, slower)
    - CPU-only machines (slowest)
"""
...
def compute_score(logits, yes_ids, no_ids):
    """
    Compute P(Yes) / (P(Yes) + P(No)) from the logit vector.

    logits: tensor of shape (vocab_size,) — raw logits for the next token.
...
def classify_single(text, language, model, tokenizer, device, yes_ids, no_ids):
    """
    ...
    score = compute_score(logits, yes_ids, no_ids)

    # Get the argmax token for reference
...
def main():
    parser = argparse.ArgumentParser(
        description="Classify political texts as law/crime-related using "
